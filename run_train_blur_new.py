...
from blur.modeling.blurnew import BlurNew
from blur.models.shared import AdaptiveInput, AdaptiveLogSoftmax
...
def train(gpu, args, config_run, config_embedder, config_transformer, config_optim):
    gpu=1
    ...
    model = BlurNew(
        ...
        predictor=AdaptiveLogSoftmax(**config_embedder.parameters()),
    )
    model.to(gpu)

    train_set = StreamDataset(
        data=corpus.train, tgt_len=config_transformer.l_q, batch_size=config_run.batch_size)
    valid_set = StreamDataset(
        data=corpus.valid, tgt_len=config_transformer.l_q, batch_size=config_run.batch_size)
...
def main():
    ...
    config_run = init_config_run(config_run=Config.from_json(config_run_path))
    ...
    config_embedder.div_value = args.div_value
    ...
    mp.spawn(
        ...
        args=(args, config_run, config_embedder, config_transformer, config_optim))
